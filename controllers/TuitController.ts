...
export default class TuitController implements TuitControllerI {
  ...
  public static getInstance = (app: Express): TuitController => {
    if (TuitController.tuitController === null) {
      ...
      app.get(
        "/api/tuits/tags/:tagName",
        TuitController.tuitController.findAllTuitsByTagID
      );
      app.post(
        "/api/users/:uid/tuits",
  ...
  findAllTuitsByTagID = async (req: Request, res: Response) => {
    ...
    const tagID = tagIDResponse[0]._id.toString();
  ...
  findTrendingTuits = async (req: Request, res: Response) => {
    try{
     ...
     let uniqueTuits =  Array.from(new Set(tuits.map(a => a.id)))
          .map(id => {
